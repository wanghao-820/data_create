...
def create_order(sku_id=None, access_token=None):
    ...
    request_payload =  {
         ...
         # 'activity':'3',
          'source':"qd_2353_activity_43"
         # 'source':"qd_2353_tk_e2c5a5"
        }
...
def add_student(access_tokens:list):
    for access_token in access_tokens:
        ...
        create_order('15427',access_token)
        get_orderid_detail()  
...
def quit_clazz_by_userids1(uid, clazz_id):
    test = Renewal()
    # 获取orderID
    order_id = str_utils.int2list(test.get_order_id_from_payment(uid,clazz_id))
    # order_id = test.get_order_id_from_payment(uid,clazz_id)
...
# def quit_clazz_by_userids(uid, clazz_id):
#     test = Renewal()
#     db = db_utils.DBUtils(name='payment')
#     # 获取orderID
#     order_id = db.select(
#         f"SELECT so.p_order_id FROM sszpay_product_attr spa LEFT JOIN sszpay_order so on so.sku_id = spa.sku_id WHERE so.user_id in ({}) AND so.status in (1,2) AND spa.attr_key = 'clazz_ids' and spa.attr_value = {} and spa.status = 1;".format(
#             uid, clazz_id))[0][0]
#
#     # order_id = test.get_order_id_from_payment(uid,clazz_id)
#
...
if __name__ == "__main__":
    ...
    # uids = [10925811, 10925812, 10925813, 10925814, 10925815, 10925816, 10925817, 10925818, 10925819, 10925820]
     sku_id = 14337
     clazz_id = db_utils.DBUtils(name='payment').select(f"select attr_value from sszpay_product_attr where attr_key = 'clazz_ids' and sku_id = {sku_id};")[0][0]
